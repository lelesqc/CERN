...
# 25 valori (lineare) per epsilon_i: 0.025 -> 0.0225
EPS_I_ARRAY=($(python3 -c "import numpy as np; print(' '.join([f'{v:.4f}' for v in np.linspace(0.025, 0.0225, 25)]))"))
# 25 valori per nu_m_i: 1.00 -> 0.90
NU_I_ARRAY=($(python3 -c "import numpy as np; print(' '.join([f'{v:.2f}' for v in np.linspace(1.0, 0.90, 25)]))"))
