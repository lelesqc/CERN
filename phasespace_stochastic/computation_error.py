...
q_sec = np.zeros((par.n_steps, n_particles), dtype=np.float32)
p_sec = np.zeros((par.n_steps, n_particles), dtype=np.float32)
sec_count = 0
...
for i in range(n_times):
    ...
    for i in range(n_times):
        if np.any(np.isnan(actions[i, :])):
            print(f"Attenzione: actions[{i},:] contiene solo nan o zeri, salto.")
            continue
    # istogramma
    hist, bin_edges = np.histogram(actions[i, :], bins=80, density=True)
...
plt.scatter(actions[-1, :], bins=100, density=True, alpha=0.5, label="Empirico")
plt.show()

times = np.linspace(0, par.dt * par.n_steps, n_times)

#mask = (times > 0.023) & (times < 0.06)
kl_div_tot = np.array(kl_div_tot)

plt.scatter(times, kl_div_tot, s=2)
plt.title(r"Kullback-Leibler divergence $\rho(I)$ - $\rho_{Boltz}$ for FCC-ee")
